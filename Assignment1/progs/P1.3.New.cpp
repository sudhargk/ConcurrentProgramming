...
class Graph{
	...
	void computeWeight(int*weight,bool* mark);
	void initmark(bool*mark);
	int computeMinDegree(int *degree);
...
void Graph::computeWeight(int* weight, bool* mark){
	cilk_for(int i=0;i<numNodes;i++){
		if(mark[i]==true){
			weight[i]=-1;
		}
...
int Graph::computeMinDegree(int* degree){
	cilk::reducer_min<int> minimumDegree;
	cilk_for(int k=0;k<numNodes;k++){
		if(degree[k]!=-1){
			minimumDegree.calc_min(degree[k]);
		}
    }
    return minimumDegree.get_value();
}
...
int* Graph::color(){
	...
	int minDegree = computeMinDegree(degree);
	int weightIdx =1;
	while(toBeWeighted>0){
		cilk::reducer_opadd<int> weighted;
		cilk_for(int vertex=0;vertex<numNodes;vertex++){
			if((!mark[vertex])&&degree[vertex]==minDegree){
				mark[vertex]=true;
				weight[vertex]=weightIdx;
				weighted+=1;
			}
		}
		toBeWeighted-=weighted.get_value();
 		computeDegree(degree,mark);
		minDegree=computeMinDegree(degree);
		weightIdx+=1;
	}	
		...
		cilk_for(int vertex=0;vertex<numNodes;vertex++){
			if(!mark[vertex]){
				...
				for(int j=0;j<adjacencyList[vertex].size();j++){
					...
					if((weight[adjVerIndex]>weight[vertex])||
						(weight[adjVerIndex]==weight[vertex]&&adjVerIndex>vertex)){
						isColorable=false;
					}
				}
				if(isColorable==true){
...
void writeOutput(int* color,int numNodes,int numColorsUsed,size_t timeelapsed){
