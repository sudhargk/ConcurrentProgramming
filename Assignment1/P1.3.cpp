	...
	while(toBeColored>0){
		...
		cilk_for(int vertex=0;vertex<numNodes;vertex++){
			if(!mark[vertex]){
				...
				cilk_for(int j=0;j<adjacencyList[vertex].size();j++){
...
void writeOutput(int* color,int numNodes,size_t timeelapsed){
	cilk::reducer_max<int> best;
	cilk_for(int k=0;k<numNodes;k++){
        best.calc_max(color[k]);
    }
    cout<<timeelapsed<<endl;
    cout<<best.get_value()+1<<std::endl;
	for(int k=0;k<numNodes;k++){
...
int main(int argc,char** argv){
	...
	int* color = mygraph->color();
	gettimeofday(&stop, NULL);
	writeOutput(color,mygraph->numNodes,stop.tv_usec-start.tv_usec);
}